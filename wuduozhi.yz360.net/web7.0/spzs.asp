...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_border">
  ...
  <tr class="t_rowh">
    <td colspan="2"><strong>操作选项</strong>：<a href="spzs.asp">信息管理</a>&nbsp;|&nbsp;<a href="spzs.asp?Action=Add">添加信息</a>&nbsp;|&nbsp;<a href="Class.asp?Layout=spzs">分类管理</a></td>
  </tr>
  <tr class="t_rowh">
    ...
    <tr align="center" class="t_row" onmouseover="this.className='t_rowh'" onmouseout="this.className='t_row'">
     ...
     <td><%=rs("DateAndTime")%></td>
      <td><a href="spzs.asp?Action=Topis&ID=<%=Rs("ID")%>">
        ...
        推荐
        <%End If%>
        </a>&nbsp;&nbsp;<a href="spzs.asp?Action=Mod&ID=<%=Rs("ID")%>">修改</a>&nbsp;<a href="spzs.asp?Action=Del&ID=<%=Rs("ID")%>" onClick="return Delete();">删除</a></td>
    </tr>
    <%
		i = i + 1
		Rs.MoveNext
...
<table border="0" align="center" cellpadding="0" cellspacing="1" class="t_border">
  ...
  <tr class="t_rowh">
    ...
    <td align="right">
      </a>&nbsp;<a href="spzs.asp?Action=Mod&ID=<%=ID%>">修改</a>&nbsp;<a href="spzs.asp?Action=Del&ID=<%=ID%>" onClick="return Delete();">删除</a>&nbsp;<a href="<%=Http_Referer%>">返回</a>&nbsp;〗</td>
  </tr>
  <tr class="t_row">
...
Sub SaveAdd()
	...
	title=CheckChar(Trim(Request.Form("title")),"视频名称",100,"012")
...
If ClassID<=0 Then ErrContent = ErrContent & "请先添加分类!"
	...
	If Conn.Execute("Select Child From Class Where ClassID="&ClassID)(0)>0 Then
		Msg "所属分类含有子目录,请选择其他分类!","spzs.asp?Action=Add",1
	End If
...
Sub SaveMod()
	...
	If Conn.Execute("Select Child From Class Where ClassID="&ClassID)(0)>0 Then
		Msg "所属分类含有子目录,请选择其他分类!","spzs.asp?Action=Mod&ID="&ID,1
	End If
